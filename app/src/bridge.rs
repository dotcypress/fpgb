...
impl BridgeSession {
    ...
    fn send_status(&self, ctx: &mut WebsocketContext<Self>) {
        ...
        let mut resp = String::from("STATUS: ");
        resp.push_str(conn_status);
        ctx.text(resp);
    }

    fn send_init(&self, ctx: &mut WebsocketContext<Self>) {
        let mut resp = String::from("INIT: FPGB v");
        resp.push_str(env!("CARGO_PKG_VERSION"));
        ctx.text(resp);
    }
    ...
    fn send_list(&self, ctx: &mut WebsocketContext<Self>) {
    ...
    fn connect(&mut self, ctx: &mut WebsocketContext<Self>, args: &[&str]) {
        ...
        if args.is_empty() {
            return self.send_error(ctx, "Port required");
        }
